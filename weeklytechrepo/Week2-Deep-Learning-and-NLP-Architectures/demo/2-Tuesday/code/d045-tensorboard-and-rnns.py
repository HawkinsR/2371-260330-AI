...
def demonstrate_tensorboard_logging():
    """
    Demo: Logging Training Metrics to TensorBoard
    Shows how to create a SummaryWriter, log scalar loss values, attach a
    model graph, and launch TensorBoard to visualize the run.
    """
    print("\n--- TensorBoard Logging Demo ---")

    try:
        from torch.utils.tensorboard import SummaryWriter
    except ImportError:
        print("[SKIP] torch.utils.tensorboard not available. Run: pip install tensorboard")
        return

    import random
    random.seed(42)

    # 1. Create a SummaryWriter — each unique run should have its own subdirectory
    #    so that TensorBoard can overlay multiple runs for comparison.
    log_dir = "runs/lstm_demo_v1"
    writer = SummaryWriter(log_dir=log_dir)
    print(f"Logging to: {log_dir}")

    # 2. Simulate a short training run and log scalar losses
    # In a real training loop, you'd log after each epoch or batch.
    print("Simulating 10 training epochs...")
    best_val_loss = float('inf')

    for epoch in range(1, 11):
        # Simulated train loss: steadily decreases
        train_loss = 1.0 / epoch + random.uniform(0, 0.05)

        # Simulated validation loss: decreases then plateaus slightly higher (realistic)
        val_loss = 0.9 / epoch + random.uniform(0, 0.08) + 0.05

        # Log both scalars under the same 'Loss' tag group for overlay in TensorBoard
    ...
    # 3. Log the model graph so TensorBoard can visualize the architecture.
    #    We reuse the model from demonstrate_lstm_pipeline by creating a fresh one.
    vocab_size, embedding_dim, hidden_dim, num_classes = 10, 16, 32, 2
    dummy_model = TextClassifierLSTM(vocab_size, embedding_dim, hidden_dim, num_classes)
    dummy_input = torch.zeros(1, 3, dtype=torch.long)  # [batch=1, seq_len=3]
    dummy_lengths = torch.tensor([3])
    writer.add_graph(dummy_model, [dummy_input, dummy_lengths])

    # 4. Always close the writer to flush pending data to disk
    writer.close()

    print(f"\nTensorBoard logs saved to: '{log_dir}'")
    print("To visualize, run the following command in your terminal:")
