...
def ingest_knowledge_base(db: MockPineconeIndex, doc_id: str, doc_text: str):
    ...
    # 1. TODO: Convert the string doc_text into a math vector
    doc_vector = None
    
    # 2. TODO: Construct the record dictionary with id, values, and metadata (must include "text": doc_text)
    record = {}
    
    # 3. TODO: Call .upsert() on the db. Pass the record in a list. Set namespace to "hr-policies"
...
def retrieve_answer(db: MockPineconeIndex, user_question: str) -> str:
    ...
    # 4. TODO: Convert the question string into a query vector
    query_vector = None
    
    # 5. TODO: Call .query() on the db. Request top_k=1 against the "hr-policies" namespace
    results = []
