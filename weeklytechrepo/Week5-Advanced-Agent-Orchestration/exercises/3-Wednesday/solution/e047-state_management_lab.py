...
def trim_chat_history(chat_history: list[DummyMessage], max_history: int = 2) -> list[DummyMessage]:
    ...
    # 1. Always retain the System Prompt at index 0
    trimmed_history.append(chat_history[0])
    
    # 2. Retain the last `max_history` messages. (Hint: Use slice notation [-max_history:])
    trimmed_history.extend(chat_history[-max_history:])
...
def secure_delete_tool(account_name: str, injected_user_id: str) -> str:
    ...
    # 3. Verify if the injected_user_id exactly matches "AUTH_USER_999"
    # Return "Account securely deleted." if true, "ERROR: Unauthorized." if false.
    if injected_user_id == "AUTH_USER_999":
        return "Account securely deleted."
...
def execute_secure_tool_call(graph_config: dict, llm_generated_args: dict):
    ...
    # 4. Extract the secure authenticated user ID from the nested graph_config dictionary
    secure_id = graph_config["configurable"]["authenticated_user_id"]
    
    # 5. Call secure_delete_tool passing both the LLM arg and the injected secure_id
    result = secure_delete_tool(
        account_name=llm_generated_args["account_name"],
        injected_user_id=secure_id
    )
