...
def build_interruptible_graph():
    ...
    # 2. Compile the graph passing the checkpointer and interrupting before "ProcessRefund" and "CancelRefund"
    graph = builder.compile(
        checkpointer=memory,
        interrupt_before=["ProcessRefund", "CancelRefund"]
    )
...
def run_approval_workflow():
    ...
    # 3. Fetch the sleeping state using graph.get_state(config)
    ...
    if sleeping_state:
        ...
        # 4. Manually override the state. Set is_approved to False, and refund_amount to 0.
        graph.update_state(
            config,
            {"is_approved": False, "refund_amount": 0}
        )
        
        print("\n[System] Phase 2: Resuming graph execution from breakpoint...")
        # 5. Resume graph execution by calling graph.invoke(None, config=config)
